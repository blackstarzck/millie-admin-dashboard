...
const generateId = () => `popupTpl${Date.now()}${Math.random().toString(36).substring(2, 7)}`;
...
const initialTemplatesData = [
    { key: 'popupTpl001', id: 'popupTpl001', name: '긴급 공지 팝업 템플릿', description: '화면 중앙에 표시되는 긴급 공지 스타일', lastModified: '2024-07-21', content: '<div class="popup-urgent"><h1>긴급 공지</h1><p>{content}</p><button>닫기</button></div>', variables: [{ key: '{content}', label: '공지 내용' }] },
    { key: 'popupTpl002', id: 'popupTpl002', name: '이벤트 홍보 팝업 템플릿', description: '이미지와 버튼이 포함된 이벤트 안내 스타일', lastModified: '2024-07-22', content: '<div class="popup-event"><img src="{imageUrl}" alt="Event"><p>{text}</p><button>자세히 보기</button><button>닫기</button></div>', variables: [{ key: '{imageUrl}', label: '이미지 URL' }, { key: '{text}', label: '이벤트 문구' }] },
];
...
export const PopupTemplateProvider = ({ children }) => {
    ...
    const updateTemplate = useCallback((key, updatedData) => {
        setTemplates(prevTemplates =>
            prevTemplates.map(tpl =>
                tpl.key === key
                    ? { ...tpl, ...updatedData, lastModified: moment().format('YYYY-MM-DD') }
                    : tpl
            )
        );
    ...
    const deleteTemplate = useCallback((key) => {
        setTemplates(prevTemplates => prevTemplates.filter(tpl => tpl.key !== key));
...
export const usePopupTemplates = () => {
    ...
    if (context === undefined) {
        throw new Error('usePopupTemplates must be used within a PopupTemplateProvider');
    }
    return context;
}; 