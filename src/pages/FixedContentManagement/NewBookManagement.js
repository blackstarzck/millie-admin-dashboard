...
const allBooks = initialBooks
  .filter(book => book.CONTENT_TYPE === '10') // 전자책만 필터링
  .map(book => ({
    ...book,
    id: book.key,
    key: book.key,
...
bookCategories.forEach(category => {
  // For demonstration, randomly assign books to categories.
  // In a real app, books would have a category property.
  booksByCategory[category] = allBooks.filter(() => Math.random() > 0.5);
});
...
const NewBookManagement = () => {
  ...
  useEffect(() => {
    const categoryBooks = booksByCategory[selectedCategory] || [];
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Typography.Paragraph>
        독자에게 노출될 '신규 도서' 목록을 관리합니다. 도서를 추가, 제외하고 순서를 변경할 수 있습니다.
      </Typography.Paragraph>
      ...
      <Flex gap="large" align="start">
        ...
        <div style={{ flex: 1 }}>
          <Flex justify="space-between" align="center" style={{ marginBottom: 12 }}>
            <Typography.Title level={5} style={{ margin: 0 }}>{selectedCategory} 신규 도서 목록</Typography.Title>
            <div>
              <Button onClick={() => setIsSearchModalVisible(true)}>도서 검색하여 추가</Button>
              <Button
                type="primary"
                onClick={() => message.success('신규 도서 목록이 저장되었습니다.')}
                style={{ marginLeft: 8 }}
              >
                저장
              </Button>
