      ...
      >
        {selectedHistoryItem && (
          <Space direction="vertical" style={{ width: "100%" }}>
            <Paragraph>
              <strong>발송 ID:</strong> {selectedHistoryItem.id}
            </Paragraph>
            <Paragraph>
              <strong>채널:</strong>{" "}
            ...
            <Paragraph>
              <strong>발송(예약) 시간:</strong>{" "}
              {moment(selectedHistoryItem.dispatchTime).format(
                "YYYY-MM-DD HH:mm:ss"
              )}
            </Paragraph>
            <Paragraph>
              <strong>상태:</strong> {getStatusTag(selectedHistoryItem.status)}
            </Paragraph>
            {(selectedHistoryItem.status === "sent" ||
              selectedHistoryItem.status === "failed") && (
              <Paragraph>
                <strong>결과:</strong> 성공{" "}
                {selectedHistoryItem.successCount?.toLocaleString() || 0} / 실패{" "}
                {selectedHistoryItem.failureCount?.toLocaleString() || 0}
                {selectedHistoryItem.status === "failed" &&
                  selectedHistoryItem.failureReason && (
                    <>
                      <br />
                      <Text type="danger">
                        실패사유: {selectedHistoryItem.failureReason}
                      </Text>
                    </>
                  )}
              </Paragraph>
            ...
            <Divider />
            <Paragraph>
              <strong>제목:</strong> {selectedHistoryItem.title}
            </Paragraph>
            <Paragraph>
              <strong>내용:</strong>
            </Paragraph>
