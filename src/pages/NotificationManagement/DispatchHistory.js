...
// --- Component ---
// Renamed NotificationHistory to DispatchHistory to match App.js
            ...
            >
                {selectedHistoryItem && (
                    <Space direction="vertical" style={{width: '100%'}}>
                         <Paragraph><strong>알림 제목:</strong> {selectedHistoryItem.title}</Paragraph>
                         <Paragraph><strong>채널:</strong> {getChannelTag(selectedHistoryItem.channel)}</Paragraph>
                         <Paragraph><strong>발송 대상:</strong> {formatTarget(selectedHistoryItem.targetType, selectedHistoryItem.targetValue)}</Paragraph>
                         ...
                         {(selectedHistoryItem.status === 'sent' || selectedHistoryItem.status === 'failed') && (
                             <Paragraph>
                                 <strong>결과:</strong> 성공 {selectedHistoryItem.successCount || 0}건 / 실패 {selectedHistoryItem.failureCount || 0}건
                             </Paragraph>
                         )}
                         {selectedHistoryItem.status === 'failed' && selectedHistoryItem.failureReason && (
                             <Paragraph><strong>실패 사유:</strong> {selectedHistoryItem.failureReason}</Paragraph>
                         )}
                         {/* TODO: Add more details like message content preview if available */}
                     </Space>
                 )}
             </Modal>
        </Space>
...
// Exporting as DispatchHistory to match App.js
